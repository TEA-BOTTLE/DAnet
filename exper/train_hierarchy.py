...
def get_arguments():
    ...
    parser.add_argument("--arch", type=str, default='vgg_HDA')
...
def train(args):
    ...
    while current_epoch < total_epoch:
        ...
        for idx, dat in enumerate(train_loader):
            ...
            img_var, root_label_var, parent_label_var, child_label_var = Variable(img), Variable(root_label), Variable(parent_label), Variable(child_label)

            logits = model(img_var)
            loss_val, loss_root_val, loss_parent_val, loss_child_val = model.module.get_loss(logits, root_label_var, parent_label_var, child_label_var)
            ...
            if global_counter % args.disp_interval == 0:
                ...
                print('Epoch: [{0}][{1}/{2}]\t'
                      ...
                      'Prec@5 {top5_root.val:.3f} ({top5_root.avg:.3f})'.format(
                       ...
                       eta_str=eta_str, eta_str_epoch=eta_str_epoch, loss=losses, loss_root=loss_root, loss_parent=loss_parent, loss_child=loss_child,
                        top1=top1, top5=top5, top1_parent=top1_parent, top5_parent=top5_parent, top1_root=top1_root, top5_root=top5_root))
        plotter.plot('parentLoss', 'train', current_epoch, loss_root.avg)
        ...
        with open(os.path.join(args.snapshot_dir, 'train_record.csv'), 'a') as fw:
            fw.write('%d \t %.4f \t %.4f \t %.4f \t  %.4f \t %.3f \t %.3f\t %.3f \t %.3f\t %.3f \t %.3f\n' % (
            current_epoch, losses.avg, loss_root.avg, loss_parent.avg, loss_child.avg, top1_root.avg,
            top5_root.avg, top1_parent.avg, top5_parent.avg, top1.avg, top5.avg))
