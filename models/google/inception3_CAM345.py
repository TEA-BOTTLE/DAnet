...
class Inception3(nn.Module):
    def __init__(self, num_classes=1000, args=None, threshold=0.6, transform_input=False):
        ...
        self.classifier4 = nn.Sequential(
            ...
            nn.ReLU(True),
            nn.Conv2d(256, 11, kernel_size=1, padding=0)
        )
        ...
        self.classifier5 = nn.Sequential(
            ...
            nn.ReLU(True),
            nn.Conv2d(384, 37, kernel_size=1, padding=0)
        )
    ...
    def forward(self, x, label=None):
        ...
        self.cam_map = cam
        root_logits = torch.mean(torch.mean(self.root_map, dim=2), dim=2)
        parent_logits = torch.mean(torch.mean(self.parent_map, dim=2), dim=2)
        child_logits = torch.mean(torch.mean(self.cam_map, dim=2), dim=2)
